...
REQUIRED_REPORT_ENGINES = ("insight", "media", "query")
...
class ReportTask:
    ...
    def __init__(
        ...
        engine_files: Optional[Dict[str, str]] = None,
        ...
        self.data_bundles = list(data_bundles or [])
        resolved_input_files = {
            name: str(path)
            for name, path in dict(input_files or {}).items()
            if name in {*REQUIRED_REPORT_ENGINES, "forum"} and str(path).strip()
        }
        explicit_engine_files = {
            engine: str(path)
            for engine, path in dict(engine_files or {}).items()
            if engine in REQUIRED_REPORT_ENGINES and str(path).strip()
        }
        self.input_files = resolved_input_files or explicit_engine_files
        self.engine_files = {
            engine: path
            for engine, path in self.input_files.items()
            if engine in REQUIRED_REPORT_ENGINES
        }
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'pdf_file_ready': bool(self.pdf_file_path),
            'pdf_file_name': self.pdf_file_name,
            'pdf_file_path': self.pdf_file_relative_path or self.pdf_file_path,
            'export_errors': list(self.export_errors),
            'data_bundle_types': [
                str(bundle.get('type'))
                for bundle in self.data_bundles
                if isinstance(bundle, dict) and bundle.get('type')
            ],
            'engine_files': dict(self.engine_files),
        }
...
def _load_task_ir(task: ReportTask) -> Dict[str, Any]:
...
def _export_markdown_artifact(task: ReportTask) -> None:
    document_ir = _load_task_ir(task)
...
def _render_argus_pdf_to_path(document_ir, pdf_path, ir_file_path):
    from .renderers.argus_pdf import ArgusPDFExporter
...
def _render_pdf_bytes_argus_first(document_ir, optimize=True, ir_file_path=None) -> bytes:
    try:
        from .renderers.argus_pdf import ArgusPDFExporter
        return ArgusPDFExporter().render_to_bytes(
            document_ir,
            ir_file_path=ir_file_path,
        )
    except Exception as exc:
        logger.warning(f"Argus PDF导出失败，回退到legacy PDFRenderer: {exc}")
        return _render_legacy_pdf_to_bytes(
            document_ir,
            optimize=optimize,
            ir_file_path=ir_file_path,
        )
...
def _write_pdf_artifact_file(task: ReportTask, pdf_path: Path, filename: str) -> None:
    task.pdf_file_path = str(pdf_path.resolve())
    task.pdf_file_relative_path = os.path.relpath(task.pdf_file_path, os.getcwd())
    task.pdf_file_name = filename


def _export_pdf_artifact(task: ReportTask, optimize: bool = True) -> None:
    document_ir = _load_task_ir(task)

    topic = document_ir.get('metadata', {}).get('topic', 'report')
    safe_topic = _safe_filename_segment(topic or 'report')
    filename = f"report_{safe_topic}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"

    output_dir = Path(settings.OUTPUT_DIR) / "pdf"
    output_dir.mkdir(parents=True, exist_ok=True)
    pdf_path = output_dir / filename

    try:
        _render_argus_pdf_to_path(document_ir, pdf_path, task.ir_file_path)
    except Exception as exc:
        logger.warning(f"Argus PDF导出失败，回退到legacy PDFRenderer: {exc}")
        pdf_bytes = _render_legacy_pdf_to_bytes(
            document_ir,
            optimize=optimize,
            ir_file_path=task.ir_file_path,
        )
        pdf_path.write_bytes(pdf_bytes)

    _write_pdf_artifact_file(task, pdf_path, filename)
    logger.info(f"导出PDF完成: {pdf_path}")


def _export_pdf_bytes_for_download(document_ir, optimize: bool = True, ir_file_path: str | None = None) -> bytes:
    return _render_pdf_bytes_argus_first(
        document_ir,
        optimize=optimize,
        ir_file_path=ir_file_path,
    )
...
def auto_export_report_artifacts(task: ReportTask) -> None:
    task.export_errors = []

    for artifact_name, exporter in (
        ("markdown", _export_markdown_artifact),
        ("pdf", _export_pdf_artifact),
    ):
        try:
            exporter(task)
            task.publish_event('artifact_ready', {
                'artifact': artifact_name,
                'task': task.to_dict(),
            })
        except Exception as exc:
            logger.exception(f"自动导出{artifact_name}失败: {exc}")
            task.export_errors.append(f"{artifact_name}: {exc}")
            task.publish_event('artifact_error', {
                'artifact': artifact_name,
                'error': str(exc),
                'task': task.to_dict(),
            })
...
def start_report_task(
    ...
    """
    创建并启动一个ReportEngine任务，供外部编排器直接调用。
    """
    global current_task
    ...
    engines_status = _resolve_report_input_files(engine_files)
    if not engines_status['ready']:
        return {
            'success': False,
            'error': f"输入文件未准备就绪: {engines_status.get('missing_files', [])}",
    ...
    task = ReportTask(
        ...
        input_files=engines_status.get('input_files') or {},
    )
    task.auto_export = auto_export
...
def _latest_md_file(directory: str) -> Optional[str]:
    if not os.path.exists(directory):
        return None

    md_files = [name for name in os.listdir(directory) if name.endswith('.md')]
    if not md_files:
        return None
...
def _resolve_report_input_files(
    engine_files: Optional[Dict[str, str]] = None,
) -> Dict[str, Any]:
    directories = {
        'insight': 'insight_engine_streamlit_reports',
    ...
    if not explicit_files:
        ...
        if not check_result.get('ready'):
            return {
                'ready': False,
                'missing_files': list(check_result.get('missing_files', [])),
                'input_files': {},
            }
        input_files.update(dict(check_result.get('latest_files') or {}))
        return {
            'ready': True,
            'missing_files': [],
    ...
    for engine, path in explicit_files.items():
        if not os.path.isfile(path):
            missing_files.append(f"{engine}: 显式输入文件不存在 ({path})")
        else:
            input_files[engine] = path

    for engine in REQUIRED_REPORT_ENGINES:
        if engine in input_files:
            continue
        latest_path = _latest_md_file(directories[engine])
        if latest_path:
            input_files[engine] = latest_path
        else:
            missing_files.append(f"{engine}: 未找到可用的最新输入文件")

    if not os.path.exists(forum_log_path):
        missing_files.append("forum: 日志文件不存在")
    else:
        input_files['forum'] = forum_log_path

    return {
        'ready': not missing_files,
        'missing_files': missing_files,
        'input_files': input_files if not missing_files else {},
    }
...
def run_report_generation(task: ReportTask, query: str, custom_template: str = ""):
    ...
    try:
        ...
        # 使用任务启动时已验证的输入文件，避免后台线程重新选择“最新”文件。
        input_files = dict(task.input_files)
        if not input_files:
            check_result = _resolve_report_input_files(task.engine_files)
            if not check_result['ready']:
                task.update_status("error", 0, f"输入文件未准备就绪: {check_result.get('missing_files', [])}")
                return
            input_files = dict(check_result.get('input_files') or {})
        ...
        # 加载输入文件
        content = report_agent.load_input_files(input_files)
        ...
        for attempt in range(1, 3):
            try:
                ...
                generation_result = report_agent.generate_report(
                    ...
                    data_bundles=task.data_bundles,
                )
                break
        ...
        if task.auto_export:
            task.publish_event('progress', {'progress': 96, 'message': '正在导出Markdown和PDF'})
            auto_export_report_artifacts(task)
...
def generate_report():
    ...
    try:
        ...
        result = start_report_task(query=query, custom_template=custom_template, auto_export=True)
...
def export_markdown(task_id: str):
    ...
    try:
        ...
        if task.markdown_file_path and os.path.exists(task.markdown_file_path):
            filename = task.markdown_file_name or os.path.basename(task.markdown_file_path)
            return send_file(
...
def export_pdf(task_id: str):
    ...
    try:
        ...
        if task.pdf_file_path and os.path.exists(task.pdf_file_path):
            filename = task.pdf_file_name or os.path.basename(task.pdf_file_path)
            return send_file(
                task.pdf_file_path,
        ...
        # 生成PDF字节流
        pdf_bytes = _export_pdf_bytes_for_download(
            document_ir,
            optimize=optimize,
            ir_file_path=task.ir_file_path,
        )
...
def export_pdf_from_ir():
    ...
    try:
        ...
        pdf_bytes = _export_pdf_bytes_for_download(
            document_ir,
            optimize=optimize,
            ir_file_path=data.get('ir_file_path'),
        )
