...
class MarkdownRenderer:
    ...
    def _render_paragraph(self, block: Dict[str, Any]) -> str:
        ...
        if self._is_metadata_paragraph(inlines) or self._is_leaked_internal_paragraph(inlines):
            return ""
    ...
    def _is_leaked_internal_paragraph(self, inlines: List[Any]) -> bool:
        """跳过明显泄漏的IR/内部元数据片段，保留正常引号文本。"""
        if not inlines or len(inlines) != 1:
            return False
        first = inlines[0]
        if not isinstance(first, dict):
        ...
        if stripped.startswith("{") and stripped.endswith("}"):
            try:
                ...
                parsed = None
            if isinstance(parsed, dict) and set(parsed.keys()) <= {"text", "marks", "type"}:
                return True
            return '"dataBundles"' in stripped
        return False
