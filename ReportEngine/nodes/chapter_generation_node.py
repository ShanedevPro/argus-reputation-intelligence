...
class ChapterGenerationNode(BaseNode):
    ...
    _USER_FACING_ENGINE_LABELS = {
        "query_engine": ENGINE_AGENT_TITLES["query"],
        "media_engine": ENGINE_AGENT_TITLES["media"],
        "insight_engine": ENGINE_AGENT_TITLES["insight"],
        "QueryEngine": ENGINE_AGENT_TITLES["query"],
        "MediaEngine": ENGINE_AGENT_TITLES["media"],
        "InsightEngine": ENGINE_AGENT_TITLES["insight"],
        "Query Agent": ENGINE_AGENT_TITLES["query"],
        "Media Agent": ENGINE_AGENT_TITLES["media"],
        "Insight Agent": ENGINE_AGENT_TITLES["insight"],
    }
    ...
    def _build_payload(self, section: TemplateSection, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        payload = {
            ...
            "reports": {
                ENGINE_AGENT_TITLES["query"]: reports.get("query_engine", ""),
                ENGINE_AGENT_TITLES["media"]: reports.get("media_engine", ""),
                ENGINE_AGENT_TITLES["insight"]: reports.get("insight_engine", ""),
            },
            "forumLogs": context.get("forum_logs", ""),
            "dataNotes": self._build_customer_facing_data_notes(
                context.get("data_bundles", [])
            ),
            "constraints": {
                "language": "zh-CN",
    ...
    def _build_customer_facing_data_notes(
        self,
        data_bundles: Any,
    ) -> List[Dict[str, Any]]:
        """把内部数据包压缩成可直接进入提示词的中文样本说明。"""
        notes: List[Dict[str, Any]] = []
        if not isinstance(data_bundles, list):
            return notes

        for bundle in data_bundles:
            if not isinstance(bundle, dict):
                continue
            if bundle.get("type") != "weibo_evidence_manifest":
                continue
            ...
            if not isinstance(keywords, list):
                keywords = []
            research_request = bundle.get("research_request") or {}
            if not isinstance(research_request, dict):
                research_request = {}

            provider = str(bundle.get("provider") or "TikHub").strip() or "TikHub"
            time_window = (
                research_request.get("timeWindow")
                or bundle.get("time_window")
                or ""
            )
            sample_summary = (
                f"数据来源与样本说明：本报告使用 {provider} 采集到的微博样本，"
                f"帖子 {counts.get('posts', 0)} 条、一级评论 {counts.get('comments', 0)} 条、"
                f"作者 {counts.get('authors', 0)} 个、关键词 {counts.get('keywords', len(keywords))} 个。"
            )
            if time_window:
                sample_summary = f"{sample_summary} 时间范围：{time_window}。"
            ...
            notes.append(
                {
                    ...
                    "keywords": [str(value) for value in keywords if str(value or "").strip()],
                }
            )
    ...
    def _stream_llm(
        ...
        **kwargs,
        ...
        """
        max_retries = self._stream_retry_max_retries()
        initial_delay = self._stream_retry_initial_delay()
        max_delay = self._stream_retry_max_delay()
        chapter_reasoning_effort = os.getenv("REPORT_ENGINE_CHAPTER_REASONING_EFFORT")
        ...
        for attempt in range(max_retries + 1):
            ...
            try:
                with self.storage.capture_stream(chapter_dir) as stream_fp:
                    ...
                    for delta in stream:
                        ...
                        if stream_callback:
                            ...
                            if max_retries > 0:
                                callback_events.append((delta, dict(meta)))
                            else:
                                self._safe_stream_callback(stream_callback, delta, meta)
                for delta, meta in callback_events:
                    self._safe_stream_callback(stream_callback, delta, meta)
                return "".join(chunks)
            except Exception as exc:
                last_error = exc
                if attempt >= max_retries or not self._is_transient_stream_error(exc):
                    raise
                ...
                logger.warning(
                    "章节流式生成遇到可重试网络错误（第 {attempt}/{total} 次）: {error}; "
                    "{delay:.1f} 秒后重试当前章节",
                    attempt=attempt + 1,
                    total=max_retries + 1,
                    error=exc,
    ...
    def _stream_retry_max_retries(self) -> int:
        """章节流式网络错误的最大重试次数。"""
        return self._read_int_env("REPORT_ENGINE_CHAPTER_STREAM_MAX_RETRIES", 2, min_value=0, max_value=5)

    def _stream_retry_initial_delay(self) -> float:
        """章节流式重试的初始等待秒数。"""
        return self._read_float_env(
            "REPORT_ENGINE_CHAPTER_STREAM_RETRY_INITIAL_DELAY",
            30.0,
            min_value=0.0,
            max_value=600.0,
        )

    def _stream_retry_max_delay(self) -> float:
        """章节流式单次重试的最长等待秒数。"""
        return self._read_float_env(
            "REPORT_ENGINE_CHAPTER_STREAM_RETRY_MAX_DELAY",
            180.0,
            min_value=0.0,
            max_value=900.0,
        )
    ...
    @classmethod
    def _is_transient_stream_error(cls, error: Exception) -> bool:
        """判断章节流式异常是否属于供应商/网络瞬态错误。"""
        message = f"{type(error).__name__}: {error}".lower()
        return any(marker in message for marker in cls._TRANSIENT_STREAM_ERROR_MARKERS)
    ...
    def _stream_retry_delay(
        ...
        *,
        ...
        """计算章节流式重试等待时间，优先尊重供应商 retry_after。"""
    ...
    def _sanitize_chapter_blocks(self, chapter: Dict[str, Any]):
        ...
        def walk(blocks: List[Dict[str, Any]] | None):
            ...
            for idx in valid_indices:
                ...
                extra_blocks = self._sanitize_block_content(block) or []
                ...
                if block_type == "list":
                    ...
                    if isinstance(nested, list):
                        walk(nested)
                if extra_blocks:
                    insertions.append((idx, extra_blocks))

            for idx, extra_blocks in reversed(insertions):
                blocks[idx + 1:idx + 1] = extra_blocks
    ...
    def _sanitize_block_content(self, block: Dict[str, Any]) -> List[Dict[str, Any]]:
        """根据类型做精细化修复，例如清理paragraph内的非法inline mark"""
        block_type = block.get("type")
        if block_type == "paragraph":
            self._normalize_paragraph_block(block)
        elif block_type == "table":
            return self._sanitize_table_block(block)
        elif block_type == "engineQuote":
            self._sanitize_engine_quote_block(block)
        return []

    def _sanitize_table_block(self, block: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if isinstance(raw_rows, list) and len(raw_rows) == 1:
            ...
            if isinstance(first_row, dict):
                ...
                if has_nested:
                    ...
                    return self._repair_table_overflow_rows(block)
        # 正常情况下，使用标准规范化
        rows = self._normalize_table_rows(raw_rows)
        block["rows"] = rows
        return self._repair_table_overflow_rows(block)

    def _repair_table_overflow_rows(self, block: Dict[str, Any]) -> List[Dict[str, Any]]:
        """拆分过宽表格行，并把误塞入表格的后续小节移回正文。"""
        rows = block.get("rows")
        if not isinstance(rows, list) or len(rows) < 2:
            return []

        expected_columns = self._infer_table_column_count(rows)
        ...
        for row in rows:
            ...
            if not isinstance(cells, list) or not cells:
                continue
            if len(cells) < expected_columns and self._is_placeholder_table_row(row):
                continue

            heading_idx = self._find_table_overflow_heading_index(cells, expected_columns)
            if heading_idx is not None:
                table_cells = cells[:heading_idx]
                overflow_cells = cells[heading_idx:]
                repaired_rows.extend(
                    self._chunk_table_row_cells(row, table_cells, expected_columns)
                )
                extracted_blocks.extend(self._table_cells_to_body_blocks(overflow_cells))
                continue

            if len(cells) > expected_columns and len(cells) % expected_columns == 0:
                repaired_rows.extend(
                    self._chunk_table_row_cells(row, cells, expected_columns)
                )
                continue
        ...
        if repaired_rows:
            block["rows"] = repaired_rows
        return extracted_blocks

    def _is_placeholder_table_row(self, row: Dict[str, Any]) -> bool:
        cells = row.get("cells") if isinstance(row, dict) else None
        if not isinstance(cells, list) or not cells:
            return True
        placeholders = {"", "--", "-", "—", "——", "N/A", "n/a"}
        return all(self._table_cell_text(cell).strip() in placeholders for cell in cells)

    def _infer_table_column_count(self, rows: List[Dict[str, Any]]) -> int:
        for row in rows:
            cells = row.get("cells") if isinstance(row, dict) else None
    ...
    def _find_table_overflow_heading_index(
        self,
        ...
        for idx, cell in enumerate(cells):
            if idx < expected_columns:
                continue
            if self._parse_numbered_heading_text(self._table_cell_text(cell)):
                return idx
        return None

    def _chunk_table_row_cells(
        self,
        row: Dict[str, Any],
        cells: List[Dict[str, Any]],
        expected_columns: int,
        ...
        for start in range(0, len(cells), expected_columns):
            chunk = list(cells[start:start + expected_columns])
            while len(chunk) < expected_columns:
                chunk.append(self._build_default_table_cell())
            new_row = dict(row)
            new_row["cells"] = chunk
            rows.append(new_row)
        return rows

    def _table_cells_to_body_blocks(self, cells: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        blocks: List[Dict[str, Any]] = []
        for cell in cells:
            text = self._table_cell_text(cell).strip()
            if not text or text in {"--", "-", "—", "——", "N/A", "n/a"}:
                continue

            heading = self._parse_numbered_heading_text(text)
            if heading:
                numbering, level = heading
                blocks.append(
                    {
                        "type": "heading",
                        "level": level,
                        "text": text,
                        "anchor": f"section-{numbering.replace('.', '-')}",
                        "numbering": numbering,
                    }
                )
            else:
                blocks.append(self._as_paragraph_block(text))
        return blocks

    def _parse_numbered_heading_text(self, text: str) -> Tuple[str, int] | None:
        match = re.match(r"^\s*(\d+(?:\.\d+)+)\s*(\S.*)$", text or "")
        if not match:
            return None
        numbering = match.group(1)
        level = min(6, numbering.count(".") + 2)
        return numbering, level

    def _table_cell_text(self, cell: Any) -> str:
        if isinstance(cell, dict):
                ...
                for block in blocks:
                    if isinstance(block, dict) and block.get("type") == "paragraph":
                        ...
                        if isinstance(inlines, list):
                            ...
                            continue
                    parts.append(self._extract_block_text(block))
                return " ".join(part.strip() for part in parts if part and part.strip())
            return self._extract_block_text(cell)
            ...
            if isinstance(text, str):
                ...
                if stripped.startswith("{") and stripped.endswith("}"):
                    ...
                    if isinstance(payload, dict) and set(payload.keys()).issubset({"type", "value"}):
                        continue
                run = dict(run)
                run["text"] = self._normalize_user_facing_text(text)
            cleaned.append(run)
        return cleaned or [self._as_inline_run("")]

    @classmethod
    def _normalize_user_facing_text(cls, text: str) -> str:
        """把内部工程角色名替换成报告可见的中文角色名。"""
        normalized = text
        refusal_markers = (
        ...
        if any(marker in normalized for marker in refusal_markers):
            return ""
        for internal_name, display_label in cls._USER_FACING_ENGINE_LABELS.items():
            normalized = normalized.replace(internal_name, display_label)
        return normalized
    ...
    def _as_inline_run(text: str) -> Dict[str, Any]:
        ...
        return {"text": ChapterGenerationNode._normalize_user_facing_text(text or ""), "marks": []}
