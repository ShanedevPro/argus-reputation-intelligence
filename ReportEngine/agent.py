...
from .renderers import ArgusHTMLRenderer, HTMLRenderer
...
class ReportAgent:
    ...
    def __init__(self, config: Optional[Settings] = None):
        ...
        self.renderer = self._initialize_html_renderer()
    ...
    def _initialize_html_renderer(self):
        renderer_name = (
            getattr(self.config, "REPORT_ENGINE_HTML_RENDERER", "argus") or "argus"
        ).strip().lower()
        if renderer_name == "argus":
            logger.info("ReportAgent: using ArgusHTMLRenderer")
            return ArgusHTMLRenderer()
        if renderer_name not in {"legacy", "html", "default"}:
            logger.warning(
                "Unknown REPORT_ENGINE_HTML_RENDERER=%s; falling back to legacy HTMLRenderer",
                renderer_name,
            )
        return HTMLRenderer()
    ...
    def generate_report(self, query: str, reports: List[Any], forum_logs: str = "",
                        ...
                        data_bundles: Optional[List[Dict[str, Any]]] = None) -> str:
        """
        ...
        参数:
            ...
            data_bundles: 可选的结构化数据包，例如微博证据清单，用于约束章节生成与确定性证据章节。
        ...
        data_bundles = list(data_bundles or [])
        ...
        try:
            ...
            if data_bundles:
                manifest_meta["dataBundles"] = data_bundles
            ...
            chapters = self._ensure_required_evidence_chapters(
                chapters,
                data_bundles=data_bundles,
            )
    ...
    def _build_generation_context(
        ...
        参数:
            ...
            data_bundles: 结构化数据包，供章节节点和确定性证据章节复用。
    ...
    def _ensure_required_evidence_chapters(
        self,
        chapters: List[Dict[str, Any]],
        data_bundles: Optional[List[Dict[str, Any]]] = None,
    ) -> List[Dict[str, Any]]:
        """当存在微博证据清单时，确定性补齐报告必须包含的证据章节。"""
        manifest = self._find_weibo_evidence_manifest(data_bundles or [])
        if not manifest:
            return chapters
        ...
        next_order = max(
            [int(chapter.get("order") or 0) for chapter in chapters if isinstance(chapter, dict)]
            or [0]
        ) + 10
        required_chapters: List[Dict[str, Any]] = []
        if "数据来源与样本说明" not in existing_titles:
            required_chapters.append(self._build_source_notes_chapter(manifest, next_order))
            next_order += 10
        if "关键证据表" not in existing_titles:
            required_chapters.append(self._build_key_evidence_chapter(manifest, next_order))
            next_order += 10
        if (
            "情绪分布与样本说明" not in existing_titles
            and self._has_sentiment_evidence(manifest)
        ):
            required_chapters.append(self._build_sentiment_notes_chapter(manifest, next_order))

        if not required_chapters:
            return chapters
        return [*chapters, *required_chapters]

    def _find_weibo_evidence_manifest(
        self,
        data_bundles: List[Dict[str, Any]],
    ) -> Optional[Dict[str, Any]]:
        for bundle in data_bundles:
            if isinstance(bundle, dict) and bundle.get("type") == "weibo_evidence_manifest":
                return bundle
        return None

    def _build_source_notes_chapter(
        self,
        ...
        provider = manifest.get("provider") or "unknown"
        research_request = manifest.get("research_request") or {}
        time_window = research_request.get("timeWindow") or manifest.get("time_window")

        source_summary = (
            f"本报告使用 {provider} 采集到的微博样本："
            f"帖子 {counts.get('posts', 0)} 条、一级评论 {counts.get('comments', 0)} 条、"
            f"作者 {counts.get('authors', 0)} 个、关键词 {counts.get('keywords', len(keywords))} 个。"
        )
        if time_window:
            source_summary = f"{source_summary} 时间范围：{time_window}。"
        boundary_summary = (
            "样本边界：仅代表本次采集到的微博样本，不代表全网或全部公众意见。"
        )

        return {
            "chapterId": "S-evidence-source",
            "title": "数据来源与样本说明",
            "order": order,
            "blocks": [
                {
                    ...
                    "level": 2,
                    "text": "数据来源与样本说明",
                    "anchor": "data-source-sample-notes",
                },
                self._paragraph_block(source_summary),
                self._paragraph_block(self._profile_framing_text(research_request)),
                self._paragraph_block(boundary_summary),
                self._paragraph_block(f"检索关键词：{self._join_text_values(keywords) or '未记录'}。"),
            ],
        }

    def _profile_framing_text(self, research_request: Dict[str, Any]) -> str:
        profile_id = str(
            (research_request or {}).get("profileId") or "generic_event_risk"
        ).strip().lower()
        compact = profile_id.replace("_", "").replace("-", "").replace(" ", "")
        if compact == "artistmanagement":
            profile_id = "artist_management"
        elif compact == "enterprisepr":
            profile_id = "enterprise_pr"
        elif compact == "genericeventrisk":
            profile_id = "generic_event_risk"

        if profile_id == "artist_management":
            return (
                "分析画像：艺人明星舆情。报告面向艺人经纪、工作室和公关团队，"
                "重点关注粉丝关系、路人观感、合作方责任边界、艺人形象损益与回应节奏。"
            )
        if profile_id == "enterprise_pr":
            return (
                "分析画像：企业公关舆情。报告面向企业公关、品牌、客服、法务和管理层，"
                "重点关注事实核验、公众信任、责任归因、客户沟通、服务补救与长期声誉影响。"
            )
        return (
            "分析画像：通用事件风险。报告面向需要判断事实、传播、舆情和处置优先级的决策者，"
            "重点关注事件边界、证据强度、风险等级、利益相关方与不确定性。"
        )

    def _build_key_evidence_chapter(
        self,
        manifest: Dict[str, Any],
        order: int,
    ) -> Dict[str, Any]:
        evidence_rows = []
        for item in (manifest.get("key_posts") or [])[:8]:
            if isinstance(item, dict):
                evidence_rows.append(("微博帖子", item))
        for item in (manifest.get("key_comments") or [])[:8]:
            if isinstance(item, dict):
                evidence_rows.append(("一级评论", item))

        table_rows = [
            self._table_row(["类型", "作者/时间", "互动", "证据摘录", "来源"])
        ]
        if evidence_rows:
            for kind, item in evidence_rows[:12]:
                table_rows.append(
                    self._table_row(
                        [
                            kind,
                            self._format_author_time(item),
                            self._format_engagement(item.get("engagement") or {}),
                            str(item.get("content") or "")[:220],
                            self._format_source_reference(item),
                        ]
                    )
                )
        else:
            table_rows.append(
                self._table_row(
                    ["样本提示", "未记录", "未记录", "证据清单未提供关键微博或评论样本。", ""]
                )
            )

        return {
            "chapterId": "S-key-evidence",
            "title": "关键证据表",
            "order": order,
            "blocks": [
                {
                    ...
                    "level": 2,
                    "text": "关键证据表",
                    "anchor": "key-evidence-table",
                },
                self._paragraph_block(
                    "下表列出本次微博样本中的关键帖子与一级评论。结论应受样本范围约束，"
                    "不得据此直接推断全网民意。"
                ),
                {
                    "type": "table",
    ...
    def _has_sentiment_evidence(self, manifest: Dict[str, Any]) -> bool:
        sentiment = manifest.get("sentiment_analysis") or {}
        if not isinstance(sentiment, dict):
            return False
        distribution = sentiment.get("sentiment_distribution") or {}
        return bool(sentiment.get("total_analyzed") or distribution)

    def _build_sentiment_notes_chapter(
        self,
        manifest: Dict[str, Any],
        order: int,
    ) -> Dict[str, Any]:
        sentiment = manifest.get("sentiment_analysis") or {}
        distribution = sentiment.get("sentiment_distribution") or {}
        total_analyzed = sentiment.get("total_analyzed") or 0
        rows = [self._table_row(["情绪类别", "样本数"])]
        if isinstance(distribution, dict) and distribution:
            for label, count in distribution.items():
                rows.append(self._table_row([f"{label}", count]))
        else:
            rows.append(self._table_row(["样本提示", "未形成有效情绪分布"]))

        summary_parts = [f"总计分析 {total_analyzed} 条文本"]
        if isinstance(distribution, dict) and distribution:
        ...
        return {
            "chapterId": "S-sentiment-sample-notes",
            "title": "情绪分布与样本说明",
            "order": order,
            "blocks": [
                {
                    ...
                    "level": 2,
                    "text": "情绪分布与样本说明",
                    "anchor": "sentiment-sample-notes",
                },
                self._paragraph_block("；".join(summary_parts) + "。"),
                self._paragraph_block(
    ...
    def _paragraph_block(self, text: Any) -> Dict[str, Any]:
        return {"type": "paragraph", "inlines": [{"text": str(text or "")}]}

    def _table_row(self, values: List[Any]) -> Dict[str, Any]:
        return {
            "cells": [
                {"blocks": [self._paragraph_block(value)]}
                for value in values
            ]
        }

    def _join_text_values(self, values: Any) -> str:
        if not isinstance(values, list):
            return str(values or "")
        return "、".join(str(value) for value in values if str(value or "").strip())

    def _format_author_time(self, item: Dict[str, Any]) -> str:
        author = item.get("author_name") or item.get("author") or "未知作者"
        created_at = item.get("created_at") or item.get("publish_time") or ""
        return f"{author} / {created_at}" if created_at else str(author)

    def _format_engagement(self, engagement: Dict[str, Any]) -> str:
        if not engagement:
            return "未记录"
        labels = [
            ("like_count", "赞"),
        ...
        parts = [
            f"{label}{engagement.get(key)}"
            for key, label in labels
            if engagement.get(key) is not None
        ]
        return " / ".join(parts) or "未记录"

    def _format_source_reference(self, item: Dict[str, Any]) -> str:
        source_id = str(
            item.get("source_id")
            or item.get("id")
        ...
        if source_id:
            return f"微博ID: {source_id}"
        parent_id = str(item.get("parent_source_id") or "").strip()
        if parent_id:
            return f"来源记录: {parent_id}"
        return "来源记录: 未记录"
