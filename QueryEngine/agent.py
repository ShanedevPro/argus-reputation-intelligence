...
class DeepSearchAgent:
    ...
    def execute_search_tool(self, tool_name: str, query: str, **kwargs) -> TavilyResponse:
        ...
        Returns:
            TavilyResponse对象
        """
        tool_name = self._normalize_search_tool_for_argus_window(tool_name)
        ...
        if tool_name == "basic_search_news":
            ...
            if start_date == end_date:
                logger.warning(
                    f"  ⚠️  search_news_by_date 时间范围为单日 {start_date}，改用基础搜索"
                )
    ...
    def _normalize_search_tool_for_argus_window(self, tool_name: str) -> str:
        """Avoid current-recency searches when Argus confirmed a historical event window."""
        if tool_name not in {"search_news_last_24_hours", "search_news_last_week"}:
            return tool_name

        window = extract_argus_time_window(self._argus_context)
        if not window:
            return tool_name
        ...
        if window_end < date.today() - timedelta(days=7):
            logger.warning(
                f"  历史研究窗口不适合使用近期新闻工具，已改用基础搜索: {tool_name} -> basic_search_news"
            )
    ...
    def _anchor_search_query(self, search_query: str, paragraph_title: str) -> str:
        """Keep generated Query searches attached to the original user topic."""
    ...
    def _filter_topic_relevant_results(self, search_results: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Drop Query results that do not mention the original topic or close terms."""
