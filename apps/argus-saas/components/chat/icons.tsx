...
export const ArgusMarkIcon = ({ size = 17 }: { size?: number }) => {
  const petal = "M8 1.6C9.5 3.27 9.73 5.2 8 7.23C6.27 5.2 6.5 3.27 8 1.6Z";
    ...
    >
      {[0, 60, 120, 180, 240, 300].map((deg, index) => (
        <path
          ...
          fill="currentColor"
          key={deg}
          opacity={index % 2 === 0 ? 0.98 : 0.72}
          transform={`rotate(${deg} 8 8)`}
        />
      ))}
      <circle cx="8" cy="8" fill="var(--card)" r="1.4" />
      <circle cx="8" cy="8" fill="currentColor" opacity="0.82" r="0.52" />
    </svg>
  );
};
