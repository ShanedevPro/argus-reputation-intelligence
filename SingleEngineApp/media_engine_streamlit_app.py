...
from utils.query_context import get_streamlit_query_context
...
def main():
    ...
    query_context = get_streamlit_query_context(st)
    auto_query = query_context.query
    ...
    if query_context.should_start_research and st.session_state.get('auto_search_query') != auto_query:
        st.session_state.auto_search_query = auto_query
        start_research = True
    elif auto_query and not query_context.should_start_research:
        st.warning("等待搜索启动信号...")
    ...
    if start_research:
        ...
        if settings.SEARCH_TOOL_TYPE == "BochaAPI":
            ...
            if not tavily_key:
                ...
                logger.error("请在您的环境变量中设置TAVILY_API_KEY")
                return
            logger.info("使用Tavily搜索API密钥")
            config = Settings(
                MEDIA_ENGINE_API_KEY=engine_key,
                MEDIA_ENGINE_BASE_URL=settings.MEDIA_ENGINE_BASE_URL,
                MEDIA_ENGINE_MODEL_NAME=model_name,
...
def execute_research(query: str, config: Settings):
    ...
    try:
        ...
        elif config.SEARCH_TOOL_TYPE == "TavilyAPI":
