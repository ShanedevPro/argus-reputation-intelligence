...
def check_dependencies() -> tuple[bool, Optional[str]]:
    ...
    try:
        ...
        if is_available:
            logger.success("✓ Legacy PDF fallback 依赖检测通过")
        else:
            logger.warning("⚠ Legacy PDF fallback 依赖缺失；Argus PDF 主路径仍会尝试生成")
            logger.info("\n" + message)
...
def save_pdf(document_ir_path: str, query: str) -> Optional[str]:
    ...
    try:
        ...
        logger.info(f"开始渲染Argus同源PDF: {pdf_path}")
        try:
...
def main():
    ...
    # 步骤 1: 检查legacy PDF fallback依赖；Argus PDF主路径不依赖Pango。
    check_dependencies()
    ...
    # 步骤 3: 生成报告
    result = generate_report(reports, query, pdf_enabled)
    ...
    # PDF默认使用Argus同源渲染，legacy依赖只影响fallback。
    if pdf_enabled:
