...
from .tools import BochaMultimodalSearch, BochaResponse, TavilyMultimodalSearch, AnspireAISearch, AnspireResponse
...
class DeepSearchAgent:
    ...
    def _normalize_search_tool_for_argus_window(self, tool_name: str) -> str:
        ...
        if tool_name not in {"search_last_24_hours", "search_last_week"}:
            return tool_name
        ...
        if window_end < date.today() - timedelta(days=7):
            logger.warning(
                f"  历史研究窗口不适合使用近期搜索工具，已改用综合搜索: {tool_name} -> comprehensive_search"
            )
    ...
    def _anchor_search_query(self, search_query: str, paragraph_title: str) -> str:
        """Keep generated Media searches attached to the original user topic."""
    ...
    def _filter_topic_relevant_results(self, search_results: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Drop Media results that do not mention the original topic or close terms."""
...
class TavilySearchAgent(DeepSearchAgent):
    ...
    def __init__(self, config: Settings | None = None):
        ...
        # 初始化搜索工具集
        self.search_agency = TavilyMultimodalSearch(api_key=self.config.TAVILY_API_KEY)
        ...
        logger.info(f"使用LLM: {self.llm_client.get_model_info()}")
        logger.info("搜索工具集: TavilyMultimodalSearch")
