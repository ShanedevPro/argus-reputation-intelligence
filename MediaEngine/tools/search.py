...
class BochaMultimodalSearch:
    ...
    BOCHA_BASE_URL = settings.BOCHA_BASE_URL or "https://api.bocha.cn/v1/web-search"
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        self.BOCHA_BASE_URL = self._normalize_base_url(self.BOCHA_BASE_URL)

    @staticmethod
    def _normalize_base_url(url: str) -> str:
        """Prefer Bocha Web Search; legacy ai-search keys may return 403."""
        normalized = (url or "https://api.bocha.cn/v1/web-search").strip()
        if normalized.rstrip("/").endswith("/v1/ai-search"):
            return normalized.rstrip("/")[:-len("ai-search")] + "web-search"
        return normalized
        ...
        if isinstance(data, dict) and isinstance(data.get("webPages"), dict):
            ...
            for item in web_results:
                ...
                final_response.webpages.append(WebpageResult(
                    ...
                    date_last_crawled=item.get("dateLastCrawled") or item.get("datePublished")
                ))

            image_results = data.get("images", {}).get("value", []) if isinstance(data.get("images"), dict) else []
    ...
    def _search_internal(self, **kwargs) -> BochaResponse:
        ...
        payload = self._build_web_search_payload(kwargs)
    ...
    def _build_web_search_payload(self, kwargs: Dict[str, Any]) -> Dict[str, Any]:
        payload: Dict[str, Any] = {
            "query": kwargs.get("query", "Unknown Query"),
            "count": kwargs.get("count", kwargs.get("max_results", 10)),
            "summary": bool(kwargs.get("answer", False)),
        }
...
class TavilyMultimodalSearch:
    """
    Tavily 搜索适配器。
    返回 BochaResponse 兼容结构，让 MediaEngine 可在稳定性测试中复用 Tavily。
    """

    def __init__(self, api_key: Optional[str] = None):
        if TavilyClient is None:
            raise ImportError("tavily-python 库未安装，请运行 `pip install tavily-python` 进行安装。")

        if api_key is None:
            api_key = settings.TAVILY_API_KEY
            if not api_key:
                raise ValueError("Tavily API Key未找到！请设置 TAVILY_API_KEY 环境变量或在初始化时提供")

        self._client = TavilyClient(api_key=api_key)
        self.provider_name = "TavilyMultimodalSearch"

    def _to_bocha_response(
        self,
        query: str,
        response_dict: Dict[str, Any],
        modal_card_hint: Optional[str] = None,
    ) -> BochaResponse:
        final_response = BochaResponse(query=query)
        final_response.answer = response_dict.get("answer")

        for item in response_dict.get("results", []) or []:
            final_response.webpages.append(
                WebpageResult(
                    name=item.get("title") or item.get("url") or "Untitled Result",
                    url=item.get("url") or "",
        ...
        for item in response_dict.get("images", []) or []:
            if isinstance(item, str):
                final_response.images.append(ImageResult(name="image", content_url=item))
                continue

            content_url = item.get("url") or item.get("image_url") or item.get("src")
            if not content_url:
                continue

            final_response.images.append(
                ImageResult(
                    name=item.get("description") or item.get("title") or "image",
                    content_url=content_url,
                    host_page_url=item.get("source_url") or item.get("host_page_url"),
        ...
        if modal_card_hint:
            final_response.modal_cards.append(
                ModalCardResult(
                    card_type=modal_card_hint,
                    content={
                        "provider": "tavily",
                        "answer": final_response.answer,
                        "webpage_count": len(final_response.webpages),
                        "image_count": len(final_response.images),
                    },
                )
            )
    ...
    def _search_internal(self, **kwargs) -> BochaResponse:
        query = kwargs.get("query", "Unknown Query")
        modal_card_hint = kwargs.pop("modal_card_hint", None)
        api_params = {key: value for key, value in kwargs.items() if value is not None}

        try:
            response_dict = self._client.search(**api_params)
            return self._to_bocha_response(query, response_dict, modal_card_hint=modal_card_hint)
        except Exception as e:
            logger.exception(f"Tavily 搜索时发生错误: {str(e)}")
            raise e

    def comprehensive_search(self, query: str, max_results: int = 10) -> BochaResponse:
        logger.info(f"--- TOOL: 全面综合搜索[Tavily] (query: {query}) ---")
        return self._search_internal(
            query=query,
            topic="general",
    ...
    def web_search_only(self, query: str, max_results: int = 15) -> BochaResponse:
        logger.info(f"--- TOOL: 纯网页搜索[Tavily] (query: {query}) ---")
        return self._search_internal(
            query=query,
            topic="general",
            max_results=max_results,
    ...
    def search_for_structured_data(self, query: str) -> BochaResponse:
        logger.info(f"--- TOOL: 结构化数据查询[Tavily兼容] (query: {query}) ---")
        return self._search_internal(
            query=query,
            topic="general",
            max_results=5,
            ...
            include_images=True,
            include_image_descriptions=True,
            modal_card_hint="tavily_structured_compat",
        )

    def search_last_24_hours(self, query: str) -> BochaResponse:
        logger.info(f"--- TOOL: 搜索24小时内信息[Tavily] (query: {query}) ---")
        return self._search_internal(
            query=query,
            topic="general",
            max_results=10,
    ...
    def search_last_week(self, query: str) -> BochaResponse:
        logger.info(f"--- TOOL: 搜索本周信息[Tavily] (query: {query}) ---")
        return self._search_internal(
            query=query,
            topic="general",
            max_results=10,
...
def load_agent_from_config():
    ...
    if settings.SEARCH_TOOL_TYPE == "TavilyAPI":
        logger.info("加载 TavilyMultimodalSearch Agent")
        return TavilyMultimodalSearch(api_key=settings.TAVILY_API_KEY)
    raise ValueError(f"未知的搜索工具类型: {settings.SEARCH_TOOL_TYPE}")
