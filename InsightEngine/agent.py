...
from utils.evidence_summary import build_evidence_summary
from utils.topic_relevance import (
    extract_topic_anchor,
    filter_relevant_items,
    rewrite_query_if_drifted,
)
from utils.argus_engine_context import (
    build_report_structure_input,
    clamp_date_range_to_argus_window,
    coerce_argus_context,
    is_datetime_within_argus_window,
)
...
class DeepSearchAgent:
    ...
    def __init__(self, config: Optional[Settings] = None):
        ...
        self.keyword_optimizer = KeywordOptimizer(
            ...
            max_keywords=self.config.KEYWORD_OPTIMIZER_MAX_KEYWORDS,
        )
    ...
    def _get_clustering_model(self):
        ...
        if SentenceTransformer is None:
            raise RuntimeError("聚类模型依赖 sentence-transformers 未安装")
    ...
    def _cluster_and_sample_results(
        ...
        if not ENABLE_CLUSTERING or np is None or KMeans is None or SentenceTransformer is None:
            logger.info("  聚类依赖未安装，按热度取样搜索结果")
            return sorted(
                results,
                key=lambda result: getattr(result, "hotness_score", 0) or 0,
                reverse=True,
            )[:max_results]
    ...
    def execute_search_tool(self, tool_name: str, query: str, **kwargs) -> DBResponse:
        ...
        if tool_name == "search_hot_content":
            ...
            if self._should_run_auto_sentiment(
                enable_sentiment, len(response.results or [])
            ):
        ...
        optimized_response = self.keyword_optimizer.optimize_keywords(
        ...
        unique_results = self._filter_argus_window_results(unique_results)
        ...
        # 检查是否需要进行情感分析
        enable_sentiment = kwargs.get("enable_sentiment", True)
        if self._should_run_auto_sentiment(enable_sentiment, len(unique_results)):
    ...
    def _filter_argus_window_results(self, results: List) -> List:
        if not results:
            return results
        filtered_results = [
            result
            for result in results
            if is_datetime_within_argus_window(
                getattr(result, "publish_time", None),
                self._argus_context,
            )
        ]
        dropped = len(results) - len(filtered_results)
        if dropped:
            logger.warning(f"  已过滤 {dropped} 条超出确认研究窗口的搜索结果")
        return filtered_results

    def _topic_anchor(self) -> str:
        return extract_topic_anchor(self._argus_context or self.state.query)

    def _should_run_auto_sentiment(self, enabled: bool, result_count: int) -> bool:
        if not enabled or result_count <= 0:
            return False
        max_runs = int(getattr(self.config, "MAX_AUTO_SENTIMENT_SEARCHES", 1) or 0)
        if max_runs > 0 and self._auto_sentiment_runs >= max_runs:
            logger.info("  自动情感分析已达到本次 Insight 运行预算，跳过后续批次")
            return False
        self._auto_sentiment_runs += 1
        return True

    def _anchor_search_query(self, search_query: str, paragraph_title: str) -> str:
        """Keep generated Insight searches attached to the original user topic."""
        rewritten_query, changed = rewrite_query_if_drifted(
            generated_query=search_query,
            original_query=self._topic_anchor(),
            paragraph_title=paragraph_title,
        )
        if changed:
            logger.warning(
                f"  查询偏离原始主题，已改写: '{search_query}' -> '{rewritten_query}'"
            )
        return rewritten_query

    def _filter_topic_relevant_results(self, search_results: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Drop Insight results that do not mention the original topic or close terms."""
        topic_anchor = self._topic_anchor()
        if not topic_anchor or not search_results:
            return search_results

        relevant_results = filter_relevant_items(topic_anchor, search_results)
        dropped = len(search_results) - len(relevant_results)
        if dropped:
            logger.warning(f"  已过滤 {dropped} 条与原始主题无关的搜索结果")
        return relevant_results

    def _clamp_search_date_range(self, start_date: str, end_date: str) -> tuple[str, str]:
        next_start, next_end, changed = clamp_date_range_to_argus_window(
            start_date,
            end_date,
            self._argus_context,
        )
        if changed:
            logger.warning(
                f"  搜索日期超出确认研究窗口，已约束: {start_date}~{end_date} -> {next_start}~{next_end}"
            )
    ...
    def research(
        self, query: str, save_report: bool = True, argus_context: str = ""
    ) -> str:
        """
        执行深度研究
        ...
        logger.info(f"{'=' * 60}")
        self._argus_context = argus_context or ""

        try:
            # Step 1: 生成报告结构
            self._generate_report_structure(query, argus_context=argus_context)
    ...
    def _generate_report_structure(self, query: str, argus_context: str = ""):
        """生成报告结构"""
        logger.info(f"\n[步骤 1] 生成报告结构...")
        ...
        report_structure_node = ReportStructureNode(
            self.llm_client,
            query,
            structure_input=build_report_structure_input(
                query, coerce_argus_context(argus_context)
            ),
        )

        # 生成结构并更新状态
        self.state = report_structure_node.mutate_state(state=self.state)
        self._limit_report_paragraphs()
    ...
    def _limit_report_paragraphs(self) -> None:
        limit = int(getattr(self.config, "MAX_PARAGRAPHS", 0) or 0)
        paragraphs = list(getattr(self.state, "paragraphs", []) or [])
        if limit > 0 and len(paragraphs) > limit:
            logger.info(f"按本地配置截取报告结构: {len(paragraphs)} -> {limit} 个段落")
            self.state.paragraphs = paragraphs[:limit]
    ...
    def _initial_search_and_summary(self, paragraph_index: int):
        ...
        search_query = self._anchor_search_query(search_query, paragraph.title)
        ...
        if search_tool in ["search_topic_by_date", "search_topic_on_platform"]:
            ...
            if start_date and end_date:
                ...
                if self._validate_date_format(
                    ...
                    start_date, end_date = self._clamp_search_date_range(
                        start_date, end_date
                    )
        ...
        if search_response and search_response.results:
            ...
            search_results = self._filter_topic_relevant_results(search_results)
        ...
        paragraph.research.add_search_results(
            search_query,
            search_results,
            parameters=dict(getattr(search_response, "parameters", {}) or {}),
        )
    ...
    def _reflection_loop(self, paragraph_index: int):
        ...
        for reflection_i in range(self.config.MAX_REFLECTIONS):
            ...
            # 生成反思搜索查询。反思是补充步骤，外部 LLM 瞬时失败不应中断已完成的主分析。
            try:
                reflection_output = self.reflection_node.run(reflection_input)
            except Exception as e:
                logger.warning(f"    反思查询生成失败，结束该段反思: {str(e)}")
                break
            search_query = reflection_output["search_query"]
            if not search_query.strip():
                logger.info("    反思未提出新搜索查询，跳过本轮反思搜索并结束该段反思")
                break
            search_query = self._anchor_search_query(search_query, paragraph.title)
    ...
    def _save_report(self, report_content: str):
        ...
        topic_anchor = self._topic_anchor()
        query_safe = "".join(c for c in topic_anchor if c.isalnum() or c in (" ", "-", "_")).rstrip()
        ...
        try:
            sidecar_path = build_evidence_summary(
                report_path=filepath,
                engine_name="insight",
                query=topic_anchor,
                paragraphs=self.state.paragraphs,
                topic_query=self._topic_anchor(),
            )
            logger.info(f"证据摘要已保存到: {sidecar_path}")
        except Exception as exc:
            logger.exception(f"证据摘要保存失败: {exc}")

        try:
            sentiment_evidence = self._extract_sentiment_evidence(self.state.paragraphs)
            sentiment_path = Path(filepath).with_suffix(".sentiment.json")
            with sentiment_path.open("w", encoding="utf-8") as f:
                json.dump(sentiment_evidence, f, ensure_ascii=False, indent=2)
            logger.info(f"情感证据摘要已保存到: {sentiment_path}")
        except Exception as exc:
            logger.exception(f"情感证据摘要保存失败: {exc}")
    ...
    def _extract_sentiment_evidence(self, paragraphs) -> Dict[str, Any]:
        """Extract reusable sample-bound sentiment evidence from search history."""
        distribution: Dict[str, int] = {}
        total = 0
        summaries: List[str] = []
        seen_sentiment_payloads: set[str] = set()

        for paragraph in paragraphs or []:
            research = getattr(paragraph, "research", None)
            history = getattr(research, "search_history", []) or []
            for entry in history:
                parameters = getattr(entry, "parameters", {}) or {}
                if not isinstance(parameters, dict):
                    continue
                sentiment = parameters.get("sentiment_analysis")
                if not isinstance(sentiment, dict):
                    continue
                try:
                    sentiment_key = json.dumps(
                        sentiment,
                        sort_keys=True,
                        ensure_ascii=False,
                        default=str,
                    ...
                    sentiment_key = repr(sentiment)
                if sentiment_key in seen_sentiment_payloads:
                    continue
                seen_sentiment_payloads.add(sentiment_key)

                local_distribution = {
                    str(label): _safe_int(count)
                    for label, count in dict(
                        sentiment.get("sentiment_distribution") or {}
                    ).items()
                }
                for label, count in local_distribution.items():
                    distribution[label] = distribution.get(label, 0) + count

                analyzed = _safe_int(sentiment.get("total_analyzed"))
                total += analyzed or sum(local_distribution.values())

                summary = str(sentiment.get("summary") or "").strip()
