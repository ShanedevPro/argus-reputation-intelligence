...
class Settings(BaseSettings):
    ...
    MAX_AUTO_SENTIMENT_SEARCHES: int = Field(
        1,
        description="自动情感分析搜索批次数上限，<=0 表示不限制",
    )
    KEYWORD_OPTIMIZER_API_KEY: Optional[str] = Field(None, description="关键词优化器API密钥")
    KEYWORD_OPTIMIZER_BASE_URL: Optional[str] = Field(
        "https://api.siliconflow.cn/v1",
        description="关键词优化器BaseUrl",
    )
    KEYWORD_OPTIMIZER_MODEL_NAME: str = Field(
        "Qwen/Qwen3-30B-A3B-Instruct-2507",
        description="关键词优化器模型名称",
    )
