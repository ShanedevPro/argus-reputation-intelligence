...
class KeywordOptimizer:
    ...
    def __init__(
        ...
        keyword_limit = (
            ...
            else getattr(settings, "KEYWORD_OPTIMIZER_MAX_KEYWORDS", 8)
        )
        self.max_keywords = max(1, int(keyword_limit or 8))

        if not self.api_key:
            logger.warning("未配置KEYWORD_OPTIMIZER_API_KEY，将使用本地关键词提取")
        ...
        try:
            if self.client is None:
                ...
                return KeywordOptimizationResponse(
                    ...
                    reasoning="关键词优化服务未配置，使用备用关键词提取",
                    success=True,
                    error_message="KEYWORD_OPTIMIZER_API_KEY is not configured",
                )
            ...
            if response["success"]:
                ...
                try:
                    ...
                    # 验证关键词质量
                    validated_keywords = self._validate_keywords(
                        keywords,
                        original_query=original_query,
                    )
                    if not validated_keywords:
                        validated_keywords = self._fallback_keyword_extraction(original_query)
    ...
    def _validate_keywords(
        ...
        for keyword in keywords:
            if isinstance(keyword, str):
                ...
                if raw_keyword.startswith(("[", "{")) or raw_keyword.endswith(("]", "}")):
                    continue
                keyword = raw_keyword.strip('"\'""''')
                keyword = re.sub(r"^[\[\{]+|[\]\}]+$", "", keyword).strip().strip('"\'""''')

                # 基本验证
                if (keyword and 
                    len(keyword) <= 20 and 
                    len(keyword) >= 1 and
                    not any(mark in keyword for mark in ("[", "]", "{", "}", "：", ":")) and
                    "选择" not in keyword and
                    "说法" not in keyword and
                    "理由" not in keyword and
                    "作为" not in keyword and
                    "最形象" not in keyword and
                    not any(bad_word in keyword for bad_word in bad_keywords) and
                    not self._is_weak_standalone_keyword(keyword) and
                    self._is_anchored_to_original_query(keyword, original_query)):
                    validated.append(keyword)
        
        if not validated and original_query:
            return self._fallback_keyword_extraction(original_query)
        return validated[: self.max_keywords]

    def _is_anchored_to_original_query(self, keyword: str, original_query: str) -> bool:
        query = re.sub(r"\s+", "", str(original_query or "").strip())
        if not query:
            return True
        ...
        query_terms = {
            term
            for term in re.split(r"[\s，。！？；：、|/()（）\"'“”]+", str(original_query or ""))
            if len(term.strip()) >= 3
        }
        return any(term in normalized or normalized in term for term in query_terms)

    def _is_weak_standalone_keyword(self, keyword: str) -> bool:
        normalized = re.sub(r"\s+", "", str(keyword or "").strip())
        if not normalized:
            return True
        if re.search(r"[A-Za-z0-9]", normalized):
            return False
        if normalized in WEAK_STANDALONE_KEYWORDS:
            return True
        if normalized in WEAK_VENUE_OR_CONTEXT_TERMS:
            return True
        return False
    ...
    def _fallback_keyword_extraction(self, original_query: str) -> List[str]:
        ...
        for token in tokens:
            ...
            if (
                ...
                and len(token) >= 2
                and not self._is_weak_standalone_keyword(token)
            ):
