...
class MediaCrawlerDB:
    ...
    def _time_range_params_for_dialect(
        self,
        start_dt: datetime,
        end_dt: datetime,
        time_type: str,
        ...
        if time_type == 'date_str' and settings.DB_DIALECT == 'postgresql':
    ...
    def search_topic_by_date(self, topic: str, start_date: str, end_date: str, limit_per_table: int = 100) -> DBResponse:
        ...
        for table, config in search_configs.items():
            ...
            param_dict['limit'] = limit_per_table
            where_clause = ' OR '.join(where_clauses)
            query = f'SELECT * FROM {self._wrap_query_field_with_dialect(table)} WHERE ({where_clause})'

            time_col, time_type = config['time_col'], config['time_type']
            time_params = self._time_range_params_for_dialect(
                start_dt,
                end_dt,
                time_type,
            )

            time_col_expr = self._wrap_query_field_with_dialect(time_col)
            time_clause = f"{time_col_expr} >= :start_time AND {time_col_expr} < :end_time"
            if table == 'zhihu_content':
                cast_type = "BIGINT" if settings.DB_DIALECT == 'postgresql' else "UNSIGNED"
                time_clause = (
                    f"CAST({time_col_expr} AS {cast_type}) >= :start_time "
                    f"AND CAST({time_col_expr} AS {cast_type}) < :end_time"
                )
            query += f" AND ({time_clause})"
            query += f" ORDER BY {self._wrap_query_field_with_dialect('id')} DESC LIMIT :limit"
            param_dict['start_time'] = time_params[0]
            param_dict['end_time'] = time_params[1]
            raw_results = self._execute_query(query, param_dict)
            for row in raw_results:
                content = (row.get('title') or row.get('content') or row.get('desc') or row.get('content_text', ''))
                time_key = row.get('create_time') or row.get('time') or row.get('created_time') or row.get('publish_time') or row.get('crawl_date')
    ...
    def get_comments_for_topic(self, topic: str, limit: int = 500) -> DBResponse:
        ...
        for table in comment_tables:
            ...
            like_select = (
                f"{self._wrap_query_field_with_dialect(like_col)} as likes"
                if like_col
                else "'0' as likes"
            )
            time_select = (
                f"{self._wrap_query_field_with_dialect(time_col)} as ts"
                if time_col in cols
                else "NULL as ts"
            )
            order_expr = (
                self._wrap_query_field_with_dialect(time_col)
                if time_col in cols
                else self._wrap_query_field_with_dialect('id')
            )

            query = (
                f"SELECT '{table.split('_')[0]}' as platform, "
                f"{self._wrap_query_field_with_dialect('content')} as content, "
                f"{author_select}, {time_select}, {like_select}, "
                f"'{table}' as source_table "
                f"FROM {self._wrap_query_field_with_dialect(table)} "
                f"WHERE {self._wrap_query_field_with_dialect('content')} LIKE :term "
                f"ORDER BY {order_expr} DESC LIMIT :limit"
            )
            raw_results = self._execute_query(query, {"term": search_term, "limit": limit})
            all_results.extend(raw_results)

        all_results.sort(
            key=lambda row: self._to_datetime(row.get('ts')) or datetime.min,
            reverse=True,
        )
    ...
    def search_topic_on_platform(
        ...
        for config in platform_configs:
            table = config['table']
            table_expr = self._wrap_query_field_with_dialect(table)
            where_clauses = []
            params: Dict[str, Any] = {}
            for idx, field in enumerate(config['fields']):
                pname = f"term_{idx}"
                where_clauses.append(
                    f"{self._wrap_query_field_with_dialect(field)} LIKE :{pname}"
                )
                params[pname] = search_term
            topic_clause = " OR ".join(where_clauses)
            query = f"SELECT * FROM {table_expr} WHERE ({topic_clause})"

            if start_dt and end_dt and 'time_col' in config:
                time_col, time_type = config['time_col'], config['time_type']
                t_params = self._time_range_params_for_dialect(start_dt, end_dt, time_type)
                
                time_col_expr = self._wrap_query_field_with_dialect(time_col)
                t_clause = f"{time_col_expr} >= :start_time AND {time_col_expr} < :end_time"
                if table == 'zhihu_content':
                    cast_type = "BIGINT" if settings.DB_DIALECT == 'postgresql' else "UNSIGNED"
                    t_clause = (
                        f"CAST({time_col_expr} AS {cast_type}) >= :start_time "
                ...
                params["start_time"] = t_params[0]
                params["end_time"] = t_params[1]
