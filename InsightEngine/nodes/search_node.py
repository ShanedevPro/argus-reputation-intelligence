...
from utils.search_node_output import build_search_node_output, is_search_node_mapping
...
class FirstSearchNode(BaseNode):
    ...
    def process_output(self, output: str) -> Dict[str, str]:
        ...
        try:
            ...
            # 验证和清理结果
            if not is_search_node_mapping(result):
                logger.warning("搜索节点输出不是结构化对象，使用默认查询")
                return self._get_default_search_query()
            ...
            return build_search_node_output(result)
...
class ReflectionNode(BaseNode):
    ...
    def process_output(self, output: str) -> Dict[str, str]:
        ...
        try:
            ...
            # 验证和清理结果
            if not is_search_node_mapping(result):
                logger.warning("反思节点输出不是结构化对象，使用默认查询")
                return self._get_default_reflection_query()

            search_query = result.get("search_query", "")

            if not search_query:
                logger.info("反思未提出新搜索查询，将跳过后续反思搜索")
                return build_search_node_output(result)
