...
def generate_pdf_with_vector_charts(document_ir, output_path, ir_file_path=None):
    """
    使用 ArgusPDFExporter 将 Document IR 渲染为与HTML同源样式的 PDF。

    Argus导出失败时回退到legacy PDFRenderer；异常时返回 None。
    ...
    try:
        logger.info("=" * 60)
        logger.info("开始生成Argus同源PDF")
        logger.info("=" * 60)

        try:
            result_path = ArgusPDFExporter().render_to_pdf(
                document_ir,
                output_path,
            ...
            result_path = PDFRenderer().render_to_pdf(
                document_ir,
                output_path,
...
def main():
    ...
    返回:
        int: 0 表示成功，非 0 表示失败。
    """
    logger.info("🚀 使用Argus同源渲染重新生成最新报告的PDF")
    logger.info("")
    ...
    if result:
        ...
        logger.info("")
        logger.info("特性说明:")
        logger.info("  ✓ 默认使用Argus HTML同源视觉系统")
        logger.info("  ✓ 通过Playwright/Chrome导出A4 PDF")
        logger.info("  ✓ 保留完整的报告视觉效果")
        logger.info("  ✓ Argus导出失败时才回退到legacy PDFRenderer")
        logger.info("")
